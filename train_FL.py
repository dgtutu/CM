...
#实现最终分类层的训练，使用 SAGA 优化器（一种高效的随机梯度下降变体）对概念特征进行分类。
# 加载预训练的 CBL 模型，提取训练、验证和测试集的概念特征，标准化后训练线性分类器。
# 保存分类器的权重（稀疏和非稀疏版本）。
parser = argparse.ArgumentParser()

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
parser.add_argument("--cbl_path", type=str, default="angle_acs/SetFit_sst2/roberta_cbm/cbl.pt")
...
if __name__ == "__main__":
    ...
    encoded_train_dataset = encoded_train_dataset[:len(encoded_train_dataset)] # ？作用是
    ...
    if 'roberta' in backbone:
        if 'no_backbone' in cbl_name:
            ...
            cbl = CBL(len(concept_set), args.dropout).to(device) #初始化一个cbl
